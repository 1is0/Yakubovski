...
__fastcall TForm1::TForm1(TComponent* Owner) : TForm(Owner) {
    ...
    ShowMessage("Изначально я сразу указывал директорию файла,\nно не знал где будет оно лежать у вас,\nтак что выберите файл Unit1.cpp\nПроверки на открытие не делал");
...
void __fastcall TForm1::Button2Click(TObject *Sender) {
	...
	if (ListBox1->ItemIndex >= min && ListBox1->ItemIndex <= max) {
		ShowMessage(
			"Необходимо выбрать строку,\nне входящую в число вставляемых строк");
		return;
	}
